...
{
    if(sourceModel()->index(source_row, ClientsModel::Column::IS_DELETED, source_parent).data(ClientsModel::Role::Read).toBool())
        return false;
...
{
    if(source_column == ClientsModel::Column::ID || source_column == ClientsModel::Column::IS_DELETED)
        return false;
